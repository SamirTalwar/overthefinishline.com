...
class EncryptedCookieCredentials(random: SecureRandom, encryptionKey: SecretKey) extends Credentials with JsonSupport {
  import EncryptedCookieCredentials._

  private val base64Decoder = Base64.getDecoder
  private val base64Encoder = Base64.getEncoder

  override def store(userCredentials: UserCredentials): Directive0 =
    setCookie(HttpCookie(
      name = CookieName,
      value = encrypt(userCredentials.toJson.compactPrint),
      path = Some("/")))

  override def retrieve: Directive1[Option[UserCredentials]] =
...
object EncryptedCookieCredentials {
