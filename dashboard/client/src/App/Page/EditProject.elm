...
htmlForNewProject repositoryNames =
  [
    div [class "col-md-6"] [
      ...
      formFor NavigateTo Location.NewProject [] (formElements "" repositoryNames)
    ]
  ]
...
htmlForExistingProject (Project username projectName repositoryNames) =
  [
    div [class "col-md-6"] [
      ...
      formFor NavigateTo (Location.EditProject username projectName) [] (formElements projectName repositoryNames)
    ]
  ]

formElements : Name -> List Name -> List (Html Message)
formElements projectName repositoryNames =
  [
    div [class "form-group"] [
      label [for "project-name"] [text "Project name"],
      input [type' "text", class "form-control", name "project-name", value projectName, required True] []
    ],
    div [class "form-group"] <| [
      p [] [text "Repositories"]
