  ...
  [
    test "App.Http.decoder: verifies that the response is authenticated" (
      let
        ...
        actual = decodeString (Http.decoder (field "thing" string)) authenticatedJson |> taskFromResult
      in
    ...
    test "App.Http.get: recognises an unauthenticated response" (
      let
        ...
        actual = decodeString (Http.decoder (succeed True)) unauthenticatedJson |> taskFromResult
      in
        assert actual (equals expected)
    ),
    ...
    test "App.Http.get: extracts partial failures from the response" (
      let
        ...
        actual = decodeString (Http.decoder (field "thing" string)) responseWithFailuresJson |> taskFromResult
      in
        assert actual (equals expected)
    )
  ]
